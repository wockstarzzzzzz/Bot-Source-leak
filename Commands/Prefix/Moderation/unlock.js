            ...
            if (this.canSendMessage(message)) {
                try {
                    const embed = new EmbedBuilder()
                        .setColor('#2b2d31')
                        .setDescription("You do not have `Manage Roles` permissions to unblock channels.");
        ...
        if (!botMember.permissions.has([PermissionsBitField.Flags.ManageRoles, PermissionsBitField.Flags.ManageChannels])) {
            ...
            if (this.canSendMessage(message)) {
                try {
                    const embed = new EmbedBuilder()
                        .setColor('#2b2d31')
                        .setDescription("I do not have the necessary permissions to unblock channels.");
                    await message.reply({ embeds: [embed] });
                } catch {}
            }
            return;
        ...
        if (!currentPermissions?.deny?.has(PermissionsBitField.Flags.SendMessages)) {
            if (this.canSendMessage(message)) {
                try {
                    const embed = new EmbedBuilder()
                        .setColor('#2b2d31')
                        .setDescription("This channel is not blocked.");
                    await message.reply({ embeds: [embed] });
                } catch {}
            }
            return;
        ...
        try {
            ...
            if (this.canSendMessage(message)) {
                try {
                    const embed = new EmbedBuilder()
                        .setColor('#2b2d31')
                        .setDescription(`🔓 ${channel} **has been unblocked**.`);
                    await message.reply({ embeds: [embed] });
                } catch {}
            }
        } catch (error) {
            ...
            if (this.canSendMessage(message)) {
                try {
                    const embed = new EmbedBuilder()
                        .setColor('#2b2d31')
                        .setDescription("An error occurred while unblocking the channel.");
                    await message.reply({ embeds: [embed] });
                } catch {}
            }
        }
