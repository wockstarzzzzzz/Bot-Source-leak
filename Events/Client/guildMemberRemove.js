...
const GoodbyeSchema = require('../../Database/Schemas/goodbye');

module.exports = {
    name: 'guildMemberRemove',
    async execute(member) {
        try {
            const data = await GoodbyeSchema.findOne({ guildId: member.guild.id });
            if (!data) return;
            ...
            const goodbyeMessage = replaceVariables(data.message, context);

            if (data.message.startsWith('{embed}')) {
                const { embed, content } = parseEmbed(goodbyeMessage);
                if (embed) {
            ...
            console.error(`Error in goodbye event: ${error.message}`);
        }
    },
};
