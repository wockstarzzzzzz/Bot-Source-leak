...
const WelcomeSchema = require('../../Database/Schemas/welcome');
...
module.exports = {
    ...
    async execute(member) {
        try {
            ...
            const autorolData = await AutorolSchema.findOne({ guildId: member.guild.id });
            if (autorolData) {
                const role = member.guild.roles.cache.get(autorolData.roleId);
                if (role) {
                    await member.roles.add(role).catch(err => console.error(`Failed to assign autorol: ${err.message}`));
                }
            }
        } catch (err) {
            console.error(`Autorol error: ${err.message}`);
        }
        ...
        try {
            ...
            context.joinPosition = position;
            context.joinPositionSuffix = getOrdinalSuffix(position);

            const welcomeMessage = replaceVariables(data.message, context);

            if (data.message.startsWith('{embed}')) {
                const { embed, content } = parseEmbed(welcomeMessage);
                if (embed) {
            ...
            console.error(`Error in welcome event: ${error.message}`);
        }
    },
};
...
function replaceVariables(template, context) {
    return template
        ...
        .replace(/{guild\.text_channels}/g, context.guild.channels.cache.filter(ch => ch.type === ChannelType.GuildText).map(ch => ch.name).join(', ') || 'N/A')
        .replace(/{guild\.text_channels_count}/g, context.guild.channels.cache.filter(ch => ch.type === ChannelType.GuildText).size.toString())
        .replace(/{guild\.voice_channels}/g, context.guild.channels.cache.filter(ch => ch.type === ChannelType.GuildVoice).map(ch => ch.name).join(', ') || 'N/A')
        .replace(/{guild\.voice_channels_count}/g, context.guild.channels.cache.filter(ch => ch.type === ChannelType.GuildVoice).size.toString())
        .replace(/{guild\.category_channels}/g, context.guild.channels.cache.filter(ch => ch.type === ChannelType.GuildCategory).map(ch => ch.name).join(', ') || 'N/A')
        .replace(/{guild\.category_channels_count}/g, context.guild.channels.cache.filter(ch => ch.type === ChannelType.GuildCategory).size.toString())
